...
private void getAlumnis() {
    ...
    reference.addValueEventListener(new ValueEventListener() {
        ...
        public void onDataChange(@NonNull DataSnapshot snapshot) {
            ...
            userAdapter = new ListAdapter(list,contextNullSafe);
    ...
    private void search (String s) {
        ...
        for (DoctorModel object : list) {
            try {
                if (object.getName().toLowerCase().contains(s.toLowerCase().trim())) {
                    mylist.add(object);
                } else if (object.getExperience().toLowerCase().contains(s.toLowerCase().trim())) {
                    mylist.add(object);
                } else if (object.getEducation().toLowerCase().contains(s.toLowerCase().trim())) {
                    mylist.add(object);
                }
            } catch (Exception e) {
                e.printStackTrace();
        ...
        ListAdapter userAdapter = new ListAdapter(mylist,getContextNullSafety());
        userAdapter.notifyDataSetChanged();
        if (recyclerView != null)
            recyclerView.setAdapter(userAdapter);
    }
